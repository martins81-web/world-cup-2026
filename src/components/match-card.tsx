...
export function MatchCard({ match, events = [], compact = false }: { match: MatchWithTeams; events?: SportsDbEvent[]; compact?: boolean }) {
  ...
  return (
    <article className={compact
      ? "max-w-full overflow-hidden rounded-md border border-black/10 bg-white p-3 shadow-sm"
      : "grid max-w-full gap-4 overflow-hidden rounded-md border border-black/10 bg-white p-4 shadow-sm sm:grid-cols-[160px_minmax(0,1fr)]"
    }>
      <MatchArtwork
        imageUrl={imageUrl}
        ...
        compact={compact}
        className={compact ? "aspect-[16/9] w-full rounded" : "aspect-[16/9] w-full rounded sm:w-40"}
      />
      <div className={compact ? "mt-3 min-w-0" : "min-w-0"}>
        ...
        <div className={compact ? "mt-3 text-xs text-black/60" : "mt-4 text-sm text-black/60"}>
          <LocalDateTime value={match.kickoffAt} /> - {notAvailable(match.venue)} - {notAvailable(match.city)}
        </div>
        <Link className={compact ? "mt-3 inline-flex rounded bg-ink px-3 py-2 text-xs font-medium text-white hover:bg-black" : "mt-4 inline-flex rounded bg-ink px-3 py-2 text-sm font-medium text-white hover:bg-black"} href={href}>
          View statistics
        </Link>
      </div>
    </article>
